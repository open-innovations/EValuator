<script>
  ...
  const svgIcon = (color = 'black') => `<svg width="7.0556mm" height="11.571mm" version="1.1" viewBox="0 0 25 41.001" xmlns="http://www.w3.org/2000/svg">
  ...
  const pointToLayer = (f, latlng) => {
    // TODO check if this is safe! Is using only connector1ratedoutputkw acceptable?
  ...
  const name = `<span>
    ...
    <span>Slow ${ svgIcon(evcolours.S) }</span>
    <span>Fast ${ svgIcon(evcolours.F) }</span>
    <span>Rapid ${ svgIcon(evcolours.R) }</span>
    <span>Ultra ${ svgIcon(evcolours.U) }</span>
  `;
</script>

{#if $location}
  <MarkerCluster { name } {options}>
