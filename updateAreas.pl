...
for($i = 0; $i < @lines; $i++){
	...
	if($i == 0){
		...
		$msoas{$m} = {};
		for($h = 1; $h < @header; $h++){
			$msoas{$m}{$header[$h]} = $cols[$h];
			if($header[$h] =~ /CD$/ && $cols[$h]){
				$lookup{$cols[$h]} = $header[$h];
			}
		}
	}
}
...
foreach $a (sort(keys(%areas))){
	if(!$lookup{$a}){
		...
		$mfile = $adir.$a."-msoas.tsv";
		if(!-e $mfile){
			print "Creating MSOA lookup for $a at $mfile\n";

			open(FILE,">",$mfile);
			foreach $m (sort(keys(%msoas))){
				#if($msoas{$m}
				if($msoas{$m}{$cd} eq $a){
					print FILE "$m\t$msoas{$m}{'MSOA11HCLNM'}\n";
				}
			}
			close(FILE);
		}
		...
		$gfile = $adir.$a.".geojson";
		if(!-e $gfile){
			print "Creating GeoJSON for $a at $gfile\n";
			$geojson = "";
			foreach $m (sort(keys(%msoas))){
				#if($msoas{$m}
				if($msoas{$m}{$cd} eq $a){
					...
					$str = join("",@lines);
					$str =~ s/(,"msoa11nm":"[^\"]*")/,"msoa11hclnm":"$msoas{$m}{'MSOA11HCLNM'}"/;
					$str =~ s/(,"msoa11nmw":"[^\"]*")//;
					print "$m\t$msoas{$m}{'MSOA11HCLNM'} - $url\n";
					$geojson .= ($geojson ? ",\n" : "").$str;
				}
			}
			if($geojson){
				...
				print "WARNING: No GeoJSON created for $a.\n";
				exit;
			}
		}
	}
